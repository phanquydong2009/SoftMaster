...
const ReviewCourseScreen = () => {
    ...
    const fetchFeedbacks = async () => {
        try {
            ...
            const response = await fetch(`http://192.168.1.4:3001/feedbackCourse/getFeedbackByCourseID/${courseId}`);
            ...
            const feedbacksWithUser = await Promise.all(
                feedbackData.map(async (feedback) => {
                    ...
                    const userResponse = await fetch(`http://192.168.1.4:3001/user/getUserByID/${feedback.userID}`);
        ...
        return (
            <View style={styles.itemReview}>
                <View style={styles.row}>
                    ...
                    <Text style={styles.txtTime}>{formatDate(item.feedbackDetail.updatedAt)}</Text>
